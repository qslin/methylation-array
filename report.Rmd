...
if(!params$skip_DM) {
  ...
  lapply(names(cpg_enrichment_result), function(x) cpg_enrichment_result[[x]] %>% dplyr::filter(overlap>10) %>% dplyr::filter(FDR<0.05) %>% write.csv(file = paste0(params$result_folder, "/Enrichment/DMP_feature_enrichment_", gsub("\\s", "_", x), ".csv"), row.names = FALSE)) %>% invisible()
...
if(!params$skip_DM) {
  ...
  lapply(names(gene_enrichment_result), function(x) gene_enrichment_result[[x]] %>% dplyr::filter(overlap>10) %>% dplyr::filter(FDR<0.05) %>% write.csv(file = paste0(params$result_folder, "/Enrichment/DMP_gene_enrichment_", gsub("\\s", "_", x), ".csv"), row.names = FALSE)) %>% invisible()
