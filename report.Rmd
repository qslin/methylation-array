...
DM_barplots <- function(plot_data, title){
  ...
  bp_ratio = nrow(plot_data)/(max(plot_data$Hypermethylation)-min(plot_data$Hypomethylation))
...
cpg_enrichment_result <- lapply(query, function(x) testEnrichment(query = x, platform = params$platform))
...
```{r gene enrichment}
gene_enrichment_result <- lapply(query, function(x) testEnrichment(x, KYCG_buildGeneDBs(x, max_distance=10000), platform = params$platform))
...
```{r gene input}
ora_genes <- lapply(query, function(x) lapply(KYCG_buildGeneDBs(x, max_distance = 10000), function(y) attributes(y)$gene_name) %>% unlist(use.names = FALSE) %>% unique())
```
